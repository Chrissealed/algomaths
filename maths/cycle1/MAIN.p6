#!/usr/bin/env perl6

use v6;

=begin pod
=NAME B<MAIN.p6> : Perl 6 executable in B<algomaths>/maths/cycle1
=AUTHOR  https://github.com/Chrissealed/algomaths.git
=VERSION 2019.05.04
=end pod

INIT {
   put 'Nice to meet you! Hope that this program will help you to make progress and have fun.';
}

sub select-a-course() {
    my $init-path = IO::Path.new("%*ENV<ALGOMATHS>/maths/cycle1");
    my Str $init-file = 'init.p6';
    loop {
        my Str $course = prompt "Veuillez choisir un cours du cycle 1 S.V.P. (bbw, fm3 ou ms3) > ";
        if ($course eq 'bbw') {
            $init-path = $init-path.add("bbw");
            last;
        } elsif ($course eq 'fm3') {
            $init-path = $init-path.add("fm3");
            last;
        } elsif ($course eq 'ms3') {
            $init-path = $init-path.add("ms3");
            last;
        } else {
            put "Identifiant du cours invalide!";
        }
    }
    for $init-path.dir(test => $init-file).Str -> $file { shell($file, cwd => $init-path) }
}

select-a-course();

END {
   put 'Bye! Hope to see you later.';
}

